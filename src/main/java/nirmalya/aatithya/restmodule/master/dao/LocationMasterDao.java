...
public class LocationMasterDao {
	...
	public List<LocationSectionModel> viewSectionListByFloor(String id) {
		...
		try {
			...
			for (Object[] m : x) {
				LocationSectionModel dropDownModel = new LocationSectionModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6],
						m[7], m[8], m[9]);
				cityList.add(dropDownModel);
			}
...
//				}
				...
				LocationRoomModel dropDownModel = new LocationRoomModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7],
...
//				}
				...
				LocationMasterModel dropDownModel = new LocationMasterModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6],
						m[7], m[8], m[9], m[10], m[11], m[12].toString(), null, null, null, null);
...
//				}
				...
				LocationMasterModel dropDownModel = new LocationMasterModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6],
						m[7], m[8], m[9], m[10], m[11], m[12].toString(), null, null, m[13], null);
				locationList.add(dropDownModel);
			}
		...
		if (id.size() > 0) {
			try {
				...
				for (Object[] m : x) {
					...
					LocationRoomModel dropDownModel = new LocationRoomModel(m[0], m[1], m[2], null, m[3], m[4], m[5],
							null, m[6], m[7], null, m[8], m[9], null, null, null);
					locationList.add(dropDownModel);
				}
				resp.setBody(locationList);
...
//					}
					...
					LocationMasterModel dropDownModel = new LocationMasterModel(m[0], m[1], m[2], m[3], m[4], m[5],
							m[6], m[7], m[8], m[9], m[10], m[11], m[12].toString(), null, null, null, null);
					locationList.add(dropDownModel);
				}
				resp.setBody(locationList);
		...
		ResponseEntity<JsonResponse<List<LocationMasterModel>>> response = new ResponseEntity<JsonResponse<List<LocationMasterModel>>>(
				resp, HttpStatus.CREATED);
		logger.info("Excel Res===" + response);
...
//						}
						...
						LocationRoomModel item = new LocationRoomModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7],
								m[8], m[9], m[10], m[11], m[12], m[13], m[14], m[15]);
						newLoc.add(item);
					}
				} else {
...
//						}
			...
			try {
				...
				if (location.getSectionId() != null && location.getSectionId() != "") {
					...
					for (Object[] m : x) {
						...
						LocationSectionModel item = new LocationSectionModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6],
								m[7], m[8], null);
						newLoc.add(item);
					}
				} else {
					...
					List<Object[]> x = em.createNamedStoredProcedureQuery("locationMasterRoutines")
							.setParameter("actionType", "addSection").setParameter("actionValue", values)
							.getResultList();
					for (Object[] m : x) {
	...
	public ResponseEntity<JsonResponse<LocationMasterModel>> editFloorMaster(String id) {
		...
		try {
			...
			for (Object[] m : x) {
				...
				LocationMasterModel item = new LocationMasterModel(m[0], m[1], m[2], null, null, null, null, null, null,
						null, null, null, null, m[3], m[4], null, null);
				newLoc.add(item);
			}
	...
	public ResponseEntity<JsonResponse<LocationSectionModel>> editSectionMaster(String id) {
		...
		try {
			...
			for (Object[] m : x) {
				...
				LocationSectionModel item = new LocationSectionModel(m[0], m[1], m[2], m[3], m[4], m[5], m[6], m[7],
						m[8], null);
				newLoc.add(item);
			}
	...
	public ResponseEntity<JsonResponse<Object>> deleteLocatioinDetails(String id) {
		...
		if (validity)
			try {
				...
				em.createNamedStoredProcedureQuery("locationMasterRoutines")
						.setParameter("actionType", "deleteFeedbackId").setParameter("actionValue", value).execute();
