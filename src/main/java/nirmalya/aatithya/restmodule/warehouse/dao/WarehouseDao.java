...
public class WarehouseDao {
	...
	@SuppressWarnings("unchecked")
	public List<DropDownModel> getWarehouseLocationList(String org,String orgDiv) {
			...
			try {
				...
				System.out.println("BinConfig="+values);
					em.createNamedStoredProcedureQuery("warehouseRoutine")
							.setParameter("actionType","saveBinConfiguration").setParameter("actionValue", values).execute();
			} catch (Exception e) {
				try {
					String[] err = serverDao.errorProcedureCall(e);
					resp.setCode(err[0]);
	...
	public ResponseEntity<JsonResponse<WirehouseRomeModel>> viewBindata(String rmId,String orgName, String orgDiv) {
		...
		try {
			...
			for (Object[] m : x) {
				...
				WirehouseRomeModel WirehouseRomeModel = new WirehouseRomeModel(m[0], m[1], m[2], m[3],m[4],m[5],m[6]);
	...
	public ResponseEntity<JsonResponse<WirehouseRomeModel>> deleteBin(String binlist,String orgName, String orgDiv) {
		...
		try {
			...
			System.out.println("value@@@@@@@@@@@@@@@@@@"+value);
			em.createNamedStoredProcedureQuery("warehouseRoutine")
					.setParameter("actionType", "deleteRoomBin").setParameter("actionValue", value).execute();
