...
public class RestAccountReceiptDao {
			...
			if(journalVoucherModel.get(0).getJournalVoucher() ==null || journalVoucherModel.get(0).getJournalVoucher() =="") {
				try {
					...
					em.createNamedStoredProcedureQuery("account_receipt_routines")
							.setParameter("actionType", "addReceiptVoucher").setParameter("actionValue", value).execute();
					// }
				} catch (Exception e) {
					try {
				...
				try {
					...
					em.createNamedStoredProcedureQuery("account_receipt_routines")
							.setParameter("actionType", "modifyReceiptVoucher").setParameter("actionValue", value).execute();
					// }
				} catch (Exception e) {
					try {
						String[] err = serverDao.errorProcedureCall(e);
	...
	public ResponseEntity<JsonResponse<List<AccountJournalVoucherModel>>> viewReceiptVoucher() {
		...
		try {
			List<Object[]> x = em.createNamedStoredProcedureQuery("account_receipt_routines")
					.setParameter("actionType", "viewReceiptVoucher").setParameter("actionValue", "").getResultList();
	...
	// editAccountInfo
		...
		@SuppressWarnings("unchecked")
		public ResponseEntity<JsonResponse<List<AccountJournalVoucherModel>>> editReceiptInfo(String id) {
			...
			try {
				...
				List<Object[]> x = em.createNamedStoredProcedureQuery("account_receipt_routines")
						.setParameter("actionType", "editReceiptInfo")
		...
		public ResponseEntity<JsonResponse<Object>> deleteJournalDetails(String id) {
			...
			if (validity)
				try {
					String value = "SET @p_receiptId='" + id + "';";
					em.createNamedStoredProcedureQuery("account_receipt_routines")
